...
define(function() {
    return function() {
        ...
        this.setTextToPanel = function(text) {
            $('.resultTextPanel').html(text)
        }

        this.updateTextEditor = function(options) {
            // update params
            ...
            // update text panel
            this.setTextToPanel(options.text)
        ...
        function loadFontList() {
            ...
            $fontsList.find('li').bind("click", function() {
                var $this = $(this)
                $chosenFont.html($this.attr('data'))
